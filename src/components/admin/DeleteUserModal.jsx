...
export default function DeleteUserModal({ user }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (!res?.success) {
        throw new Error(res?.error || res?.message || 'Failed to delete user');
      }
          ...
          <Modal.Dialog className="sm:max-w-[380px]">
            ...
            <Modal.Body>
              <p className="text-sm text-default-500">
                Are you sure you want to delete <span className="font-semibold text-foreground">{user?.name}</span>? This will permanently remove their account.
              </p>
            </Modal.Body>
